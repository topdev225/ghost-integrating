...
export class NewGhostScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      scrollTabIndex: 0,
  ...
  onPressInbox(inbox, labelName, photoURL) {
    ...
    NavigatorService.navigate('chat', { inbox, labelName, onBack: () => this.fetchInboxData() });
  }
    ...
    return (
      <View style={styles.container}>
        <ScrollView contentContainerStyle={styles.scrollView}>
          {inboxData.map((inbox, index) => this.renderInboxItem(inbox, index, 'unknown'))}
        </ScrollView>
      </View>
    );
  }
...
}), mapDispatchToProps)(NewGhostScreen);
